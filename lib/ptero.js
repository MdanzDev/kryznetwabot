...
async function getEgg(eggId) {
    const r = await axios.get(`${BASE}/application/eggs/${eggId}?include=variables`, {
        headers: headers(ADMIN_KEY), timeout: 15000, validateStatus: () => true
    });
    if (r.status !== 200) throw new Error(r.data?.errors?.[0]?.detail || `Failed to fetch egg ${eggId} (${r.status})`);
...
async function createServer(name, ownerId, eggId, nodeId, memory, disk, cpu) {
