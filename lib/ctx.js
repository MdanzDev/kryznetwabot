...
class Ctx {
    ...
    get quoted() {
        ...
        const raw = context.quotedMessage;
        const message = Baileys.extractMessageContent(raw) || {};
        ...
        return {
            ...
            message,
            messageType: helper.getMessageType(raw),
            key: {
                remoteJid: chat,
                id: context.stanzaId,
    ...
    async target(priority = ["quoted", "mentioned", "text"]) {
        const strategies = {
            ...
            text: () => {
                ...
                return number && {
                    id: number + Baileys.S_WHATSAPP_NET,
                    source: "text"
                };
            },
            text_group: () => {
        ...
        for (const type of priority) {
            ...
            if (result) {
                if (Baileys.isPnUser(result.id)) {
                    try {
                        result.id = (await this.core.findUserId(result.id)).lid;
                    } catch {}
                }
                return result;
            }
    ...
    async stopTyping() {
        await this._client.sendPresenceUpdate("paused", this.id);
    }
    ...
    async sendMessage(jid, content, options = {}) {
        // Resolve LID → PN JID for delivery. WA can receive from LIDs but
        // cannot deliver TO LID addresses — messages silently fail.
        const resolved = this._resolveSendJid(jid);
        return await this._self.sendMessage(resolved, content, options);
    }
    async reply(content, options = {}) {
        const resolved = this._resolveSendJid(this.id);
        return await this._self.sendMessage(resolved, content, {
            ...options,
            quoted: this._msg
        });
    }
    // LID JIDs (xxx@lid) can receive but not deliver. Resolve to PN JID
    // (xxx@s.whatsapp.net) using the sender's phone number when available.
    _resolveSendJid(jid) {
        if (jid && typeof jid === "string" && jid.includes("@lid")) {
            // Try sender's PN JID first
            const pn = this._sender?.jid;
            if (pn && pn.includes("@s.whatsapp.net")) return pn;
            // Fallback: try to use the LID's associated PN from message participant
            const participant = this._msg?.key?.participant;
    ...
    async delete(key) {
        return await this.deleteMessage(this.id, key);
    }
    async editMessage(jid, newText, key) {
        return await this._self.sendMessage(jid, {
            text: newText,
            edit: key
        });
    }
    async edit(newText, key) {
        return await this.edit(this.id, newText, key);
    }
