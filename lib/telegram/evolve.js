...
const ALLOWED_EXT = [".js", ".json", ".md", ".txt", ".env", ".gitignore", ".yml", ".yaml", ".sh", ".conf", ".service", ".html", ".css"];

// Extra server-wide paths Alya may read/patch (as root) — nginx, pm2, system.
// Everything else outside PROJECT_ROOT stays blocked.
const SERVER_ALLOWED = [
    "/etc/nginx/",
    "/root/.pm2/",
    "/etc/systemd/system/"
];
const SERVER_ALLOWED_FILES = [
    "/etc/hosts",
    "/etc/resolv.conf"
];

function isServerPathAllowed(resolved) {
    if (SERVER_ALLOWED.some(p => resolved === path.resolve(p) || resolved.startsWith(path.resolve(p) + path.sep))) return true;
    if (SERVER_ALLOWED_FILES.includes(resolved)) return true;
    return false;
}
...
function isPathSafe(targetPath) {
    ...
    // Server-wide allowlist takes precedence (nginx/pm2/systemd configs)
    if (isServerPathAllowed(resolved)) return true;
    // Must be inside project root
    const rel = path.relative(PROJECT_ROOT, resolved);
    if (rel.startsWith("..") || path.isAbsolute(rel)) return false;
    // Block sensitive paths — but ALLOW state/extracted/ (zip extraction output)
    if (/\.git\/|node_modules\/|database\/alya\.db/.test(rel)) return false;
    if (/^state\//.test(rel) && !/^state\/extracted\//.test(rel)) return false;
    if (rel === "state") return false;
    return true;
}
...
class Evolve {
    ...
    listFiles(targetDir = null) {
        // "plugins" shortcut -> the actual plugins dir
        if (targetDir === "plugins" || targetDir === "/plugins" || targetDir === "plugin") {
            targetDir = path.relative(PROJECT_ROOT, this.pluginsDir);
        }
        // Absolute paths (server allowlist) resolve as-is; relative to project root
        const dir = targetDir
            ? (path.isAbsolute(targetDir) ? targetDir : path.join(PROJECT_ROOT, targetDir))
            : PROJECT_ROOT;
        if (!isPathSafe(dir)) return { error: "Path di luar area yang diizinkan." };
        if (!fs.existsSync(dir)) return { error: `Directory "${targetDir}" tidak ada.` };
        try {
    ...
    readFile(targetPath) {
        const full = path.isAbsolute(targetPath) ? path.resolve(targetPath) : path.join(PROJECT_ROOT, targetPath);
