...
class TelegramBot {
    ...
    // ---------- Web search (Wikipedia smart + DuckDuckGo, zero-key) ----------
    ...
    async webSearch(query) {
        ...
        const bornMatch = query.match(/(?:hari|tanggal|tgl)?\s*(?:lahir|ulang ?tahun|born|birthday)\s+(?:si(?:st|r)?\s+)?(.+?)$/i)
        ...
        for (const lang of ["id", "en"]) {
            for (const att of attempts) {
                ...
                const extract = await this.wikiExtract(best.hit.title, lang);
                if (extract) {
                    // For birth-date questions prefer the FULL intro — date sits
                    // in the first parenthesis; keep it verbatim.
                    return { source: `wikipedia-${lang}`, title: best.hit.title, text: truncate(extract, 1200) };
                }
            }
        }
        ...
        this.tg.call("sendMessage", { chat_id: chatId, text: frames[0] })
            .then((r) => {
                ...
                handle.timer = setInterval(() => {
                    ...
                    }).catch(() => { /* edit failed — keep animation going silently */ });
                    i++;
                }, 2400);
                handle.timer.unref?.();
            })
    ...
    async handle(msg, prompt) {
        ...
        const typingKeepalive = setInterval(() => {
            if (thinking?.stopped) { clearInterval(typingKeepalive); return; }
        ...
        // Thinking animation -> edit into final reply (feels instant)
        clearInterval(typingKeepalive);
        if (thinking) {
