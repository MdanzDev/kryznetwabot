...
class AIClient {
    ...
    async chat(model, messages, { timeoutMs = 90000, maxTokens = 4000 } = {}) {
    ...
    async chatWithFallback(tier, messages, opts = {}) {
        if (this.forcedModel) {
            ...
            return { reply: r.content, model: this.forcedModel, finishReason: r.finishReason, usage: r.usage };
        }
        ...
        for (const model of list) {
            try {
                ...
                return { reply: r.content, model, finishReason: r.finishReason, usage: r.usage };
            } catch (error) {
...
class TelegramBot {
    constructor(bot, tgConfig) {
        ...
        // Each platform gets its OWN independent schedule.
        const tgOwnerIds = tgConfig.ownerIds || [];
        this.initiated = new InitiatedMessenger(
            ...
            tgOwnerIds.map(id => ({
                ...
                chatId: String(id),
                platform: "telegram",
                sender: (text) => this._sendInitiatedMessage(String(id), text)
            }))
        );
    ...
    async handle(msg, prompt) {
        ...
        const maxTokens = wantsLong ? 8000 : 4000;
        ...
        if (isVoiceReply) {
            ...
            // No cap — follow AI's natural paragraph/sentence structure
            const parts = this.humanizer.splitResponse(finalReply);
            ...
            // Send remaining parts with typing delays — background, don't block polling
            if (parts.length > 1) {
                (async () => {
