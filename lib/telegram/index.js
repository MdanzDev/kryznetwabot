...
class TelegramBot {
    ...
    // ---------- Text-to-speech (Google TTS, zero-key) ----------
    async speak(chatId, text) {
        // gTTS max ~200 chars per request — chunk on sentence boundaries
        ...
        for (const chunk of chunks.slice(0, 5)) { // cap: 5 segments
    ...
    async handle(msg, prompt) {
        ...
        const finalReply = truncate(result.reply.trim(), 3900);
        await this.tg.sendMessage(chatId, finalReply);
        // Voice replies: /voice mode on, or answering a voice note
        const voiceMode = this.state.get("voiceMode", {})[String(chatId)];
        if (voiceMode || msg.voice) {
