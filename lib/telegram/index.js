...
class TelegramBot {
    ...
    _costReport() {
        // cost units = tokens x multiplier; real RM depends on the provider plan rate
        const multipliers = {
            "auto": 1, "glm-5.1": 1, "hy3": 1.4, "kimi-k2.7-code": 1,
            "deepseek-v4-pro": 1.15, "glm-5.2": 1.25, "deepseek-v4-mod": 1.25,
            "glm-5.3": 2, "glm-5.3-flash": 2, "deepseek-v4-flash": 2,
            "deepseek-v4-flash-vision-exp": 2.5, "kimi-k3": 2, "kimi-k3-mod": 2,
            "gpt-5.6": 5, "claude-opus-5": 12, "gpt-5.6-luna": 5, "gpt-5.6-terra": 10
        };
        let totalUnits = 0;
        const lines = Object.entries(stats.byModel).map(([m, e]) => {
            const mult = multipliers[m] || 1;
            const units = e.tokens * mult;
            totalUnits += units;
            return `  ${m}: ${e.calls}x · ${e.tokens} tok · ${mult}x → ${units.toLocaleString("id-ID")} unit`;
        }).join("\n") || "  (belum ada)";
        return (
            `❖ Cost report (lifetime)\n` +
            `Total: ${stats.calls} calls · ${stats.tokens.toLocaleString("id-ID")} tokens\n` +
            `Weighted cost: ${totalUnits.toLocaleString("id-ID")} unit (tokens × multiplier)\n\n` +
            `Per model:\n${lines}\n\n` +
            `Note: 1 unit = 1 token @ 1x. Harga RM asli tergantung rate plan provider kamu.`
        );
    }

    // Multi-brain debate: strong models answer in parallel, cheap model synthesizes
    async _debate(messages, maxTokens) {
        const brains = ["glm-5.3", "deepseek-v4-pro", "kimi-k3"];
