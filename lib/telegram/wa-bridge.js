            ...
            try {
                ...
                if (cand && typeof cand === "object" && cand.action) {
                    if (cand.action === "remember" && cand.fact) {
                        ...
                        // Built-in Playwright — no npx, correct chromium, buffer back
                        const { getBrowser } = require("./browser");
                        const url = String(cand.url).trim();
                        await ctx.reply({ text: `(｡･ω･｡) Screenshot ${url.slice(0, 60)}... bentar ya~` });
                        const r = await getBrowser().screenshot(url, { fullPage: Boolean(cand.fullPage) });
                        if (r.error) {
                            replyText = `(╥﹏╥) Screenshot gagal: ${r.error.slice(0, 200)}`;
                        } else {
                            // Save to disk so it can be re-used (sticker etc.), then send
                            ...
                            this.lastScreenshot = fpath;
                            await this.tg.bot.sendMessage(ctx.id, { image: r.buffer, caption: `📸 ${r.title || url}` });
                            return true;
                        }
                    } else if (cand.action === "send_file" && isOwner && cand.path) {
                        // Send any file from disk as image/sticker/document
                        const fs = require("node:fs");
                        const fpath = String(cand.path).trim();
                        if (!fs.existsSync(fpath)) {
                            replyText = `(╥﹏╥) File ${fpath} tak wujud la sayang...`;
                        } else {
                            const buf = fs.readFileSync(fpath);
                            const as = String(cand.as || "image").toLowerCase();
                            if (as === "sticker") {
                                await this.tg.bot.sendMessage(ctx.id, { sticker: buf });
                            } else if (as === "document") {
                                await this.tg.bot.sendMessage(ctx.id, { document: buf, mimetype: "application/octet-stream", fileName: fpath.split("/").pop() });
                            } else {
                                await this.tg.bot.sendMessage(ctx.id, { image: buf, caption: cand.caption || fpath.split("/").pop() });
                            }
                            return true;
                        }
                    }
