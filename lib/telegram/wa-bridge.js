...
class WaAlya {
    ...
    async _handleInner(ctx, isGroup) {
        ...
        try {
            ...
            // Cap bubbles at 8 per reply when queued — prevents 75s bubble
            // chains from blocking other groups in the queue. Remaining text
            // gets merged into the last bubble.
            const MAX_BUBBLES = 8;
            if (parts.length > MAX_BUBBLES) {
                parts[MAX_BUBBLES - 1] = parts.slice(MAX_BUBBLES - 1).join("\n");
