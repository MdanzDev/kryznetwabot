...
class WaAlya {
    ...
    async handle(ctx, { isGroup }) {
        try {
            ...
            // Send the FIRST part synchronously (so the user sees immediate response),
            // then send remaining parts in the background — don't block the event loop
            // for 40-125s while sending 50 bubbles with delays. This was causing ALL
            // subsequent messages (including commands) to queue and not respond.
            ...
            if (parts.length > 0) await sendPart(0);

            // Remaining bubbles: background, don't block the message handler
            if (parts.length > 1) {
                (async () => {
                    for (let i = 1; i < parts.length; i++) {
                        await sendPart(i);
                    }
                })().catch(e => console.log("[WA-Alya] background send error:", e.message));
            }
