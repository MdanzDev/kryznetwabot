...
class WaAlya {
    ...
    async _buildMediaContent(ctx) {
        ...
        // Image: download -> base64 -> vision model content
        if (type === "imageMessage" || (type === "viewOnceMessageV2" && msg.message?.viewOnceMessageV2?.message?.imageMessage)) {
            try {
                ...
                if (Buffer.isBuffer(buffer) && buffer.length > 100) {
                    out.hasImage = true;
                    out.userContent = [
                        { type: "text", text: caption || "Describe / react to this image with gaya Alya." },
                        { type: "image_url", image_url: { url: `data:image/jpeg;base64,${buffer.toString("base64")}` } }
                    ];
                }
            } catch (e) {
                console.log(`[WA-Alya] image download failed: ${e.message}`);
            }
            return out;
        }
    ...
    async handle(ctx, { isGroup }) {
        try {
            ...
            const system =
                ...
                (isOwner
                    ? "Kamu punya akses ROOT ke server Kryz dan BISA menjalankan perintah terminal.\n" +
            ...
            if (!media.hasImage) messages.push(...history.slice(-8));
            messages.push({ role: "user", content: media.userContent || body });

            // Vision tier when an image is attached; cheap tier otherwise (token saving)
            const result = await this.tg.ai.chatWithFallback(
                media.hasImage ? "vision" : "cheap",
                messages,
                { maxTokens: media.hasImage ? 1500 : 1200 }
            );
