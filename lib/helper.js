...
function getDb(collection, jid) {
    ...
    const ensure = (collection, jid, isMatch) => {
    ...
    if (name === "bot") return ensure(collection, "bot", (bot) => bot.jid === "bot");
    if (name === "users" && Baileys.isLidUser(jid)) return ensure(collection, jid, (user) => Baileys.areJidsSameUser(user.jid, jid));
    if (name === "groups" && Baileys.isJidGroup(jid)) return ensure(collection, jid, (group) => Baileys.areJidsSameUser(group.jid, jid));
    return null;
}
