...
class Client {
    ...
    _getSender(key) {
        ...
        return {
            jid: fromMe ? Baileys.jidNormalizedUser(user?.id) : jids.find(jid => Baileys.isPnUser(jid)),
            lid: fromMe ? Baileys.jidNormalizedUser(user?.lid) : jids.find(jid => Baileys.isLidUser(jid))
        };
    }
