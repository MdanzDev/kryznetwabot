   ...
   async _getSender(key) {
    ...
    for (const message of event.messages) {
        ...
        await this._cacheStatusMessage(message);
                ...
                // NOTE: object spread ({...this}) strips the Client prototype —
                // ctx.bot would lose forceCommand/getDb/sendMessage. Instead,
                // build the self object ON the Client prototype and copy own props.
                const cmdSelf = Object.assign(
                    Object.create(Object.getPrototypeOf(this)),
                    this,
                    {
                        m: { ...message, body },
                        sender: { ...sender, pushName: message.pushName }
                    }
                );
                await Commands(cmdSelf, this._runMiddlewares.bind(this));
            }
        });
        ...
        // Baileys emits these with an EMPTY array during groupFetchAllParticipating
        // (reconnects, /menu) — guard instead of crashing on `event.id` of undefined.
        this.core.ev.on("groups.update", async (events) => {
            for (const event of (Array.isArray(events) ? events : [events])) {
                if (event?.id) await this._cacheGroupMetadata(event.id).catch(() => {});
            }
        });
        this.core.ev.on("groups.upsert", async (events) => {
            for (const event of (Array.isArray(events) ? events : [events])) {
                if (event?.id) await this._cacheGroupMetadata(event.id).catch(() => {});
            }
        });
    ...
    // Resolve "@Name" / "@number" tokens in text against group members'
    // pushNames, returning { text, mentionedJid } with real mention metadata.
    // This lets AI-triggered commands ("kick @Arisya") target real JIDs —
    // a plain text "@Arisya" can't be resolved by target() otherwise.
    async _resolveMentions(jid, text) {
        const out = { text, mentionedJid: [] };
        if (!text || !Baileys.isJidGroup(jid)) return out;
        const tokens = text.match(/@[\p{L}][\p{L}\d_ .'-]{0,30}|@\d{5,}/gu) || [];
        if (!tokens.length) return out;
        let participants = [];
        try {
            const meta = await this.core.groupMetadata(jid);
            participants = (meta?.participants || [])
                .map(p => p.jid || p.id)
                .filter(Boolean)
                .map(j => Baileys.jidNormalizedUser(j));
        } catch {
            return out;
        }
        if (!participants.length) return out;
        const userColl = this.db.getCollection("users");
        let newText = text;
        for (const token of tokens) {
            const name = token.slice(1).trim();
            if (!name) continue;
            const nameLower = name.toLowerCase();
            const hit = participants.find(pj => {
                const pjDigits = String(pj).split("@")[0];
                if (nameLower === pjDigits.toLowerCase()) return true;
                const pn = String(helper.getDb(userColl, pj)?.pushName || "").toLowerCase();
                return pn === nameLower
                    || pn.startsWith(nameLower)
                    || pn.split(/\s+/).includes(nameLower);
            });
            if (!hit) continue;
            const hitDigits = String(hit).split("@")[0];
            newText = newText.replace(token, "@" + hitDigits);
            if (!out.mentionedJid.includes(hit)) out.mentionedJid.push(hit);
        }
        out.text = newText;
        return out;
    }
    ...
    async forceCommand(jid, command, text = "", sender) {
        // The forced body MUST carry the command prefix, otherwise
        // parseCommand() treats it as plain text and Commands() exits early.
        // Also accept an already-prefixed command ("forceCommand(jid, '/menu')").
        const hasPrefix = typeof command === "string" && command.startsWith("/");
        let body = text
            ? (hasPrefix ? `${command} ${text}` : `/${command} ${text}`)
            : (hasPrefix ? command : `/${command}`);
        // Resolve "@Name" tokens to real group-member mentions
        const { text: resolvedBody, mentionedJid } = await this._resolveMentions(jid, body);
        body = resolvedBody;
        const fakeMsg = {
            key: {
                remoteJid: jid,
        ...
        // Same prototype fix as the dispatch path — ctx.bot must keep methods.
        const cmdSelf = Object.assign(
            Object.create(Object.getPrototypeOf(this)),
            this,
            { m: fakeMsg, sender, force: true }
        );
        await Commands(cmdSelf, this._runMiddlewares.bind(this));
    }
    ...
    async launch() {
        ...
        this.core = Baileys.default({
            auth: this.state,
            logger: this.logger,
            // Hard caps so a wedged upload/query can never hang a send forever.
            // This Baileys fork has NO defaults for either: mediaUploadTimeoutMs=undefined
            // makes uploadWithNodeHttp set `timeout: undefined` (socket can idle forever,
            // and it never rejects if the server stalls mid-body), and the media_conn IQ
            // inside refreshMediaConn runs query() with no outer timeout.
    ...
    async _createSendMessage(jid, content, options = {}) {
        ...
        // Media sends (upload path) can still wedge below Baileys' own timeouts
        // (e.g. media_conn IQ stuck → media-conn keyed mutex held forever → every
        // later media send queues silently; the 7:48pm anime hang showed exactly
        // this: text sends kept working, media never resolved). Watchdog: if a
        // media send hasn't settled in 12 minutes, force a reconnect so the whole
        // socket state (mediaConn promise + mutexes + pending IQ) is rebuilt.
        const isMedia = ["image", "video", "audio", "sticker", "document", "ptv"]
            .some(k => k in content);
        if (!isMedia) return await this.core.sendMessage(jid, content, options);
        let timer;
        const watchdog = new Promise((_, reject) => {
            timer = setTimeout(() => reject(new Error("Media send melebihi 12 minit — sambungan ditukar, cuba lagi.")), 12 * 60 * 1000);
        });
        try {
            return await Promise.race([this.core.sendMessage(jid, content, options), watchdog]);
        } catch (e) {
            this.logger.warn({ err: e?.message }, "media send watchdog tripped, forcing reconnect");
            try { this.core.end(new Error("media send watchdog")); } catch {}
            throw e;
        } finally {
            clearTimeout(timer);
        }
