...
function extractAction(text) {
    ...
    if (start >= 0 && end > start) {
        ...
        // Last resort: regex-recover the known fields even if the JSON is malformed
        const am = t.match(/"action"\s*:\s*"([a-z_]+)"/i);
        if (am) {
            ...
            for (const key of ["cmd", "command", "args", "url", "path", "as", "fact", "message", "caption", "fullPage"]) {
                const v = grab(key);
                if (v !== undefined) action[key] = v;
            }
            return action;
        }
    }
...
async function handleKryzz(bot, ctx, prompt) {
    ...
    if (!isNewGoal && fsSession.existsSync(sessionFile)) {
        try {
            ...
            history = [`USER REQUEST (GOAL): ${prompt}`];
        }
    } else {
        history = [`USER REQUEST (GOAL): ${prompt}`];
